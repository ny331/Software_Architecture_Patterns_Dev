...
namespace ExploreCalifornia.EventSourcing.DataAccess
{
    public class EventSourcedRepository<T> : IEventSourcedRepository<T> where T : class, IEventSourced
    {
        ...
        public T Get(Guid id)
        {
            ...
            {
                var eventRecords = connection.Query<EventRecord>(
                    "SELECT Id, EntityId, EntityType, Version, " +
                    "Payload, Timestamp " +
                    "FROM Event " +
                    "WHERE EntityType = @EntityType " +
                    "AND EntityId = @EntityId " +
                    "ORDER BY Version", new 
                    { EntityType = typeof(T).Name, EntityId = id })
                    .ToList();

                var versionedEvents = eventRecords
                    .Select(Deserialize)
                    .ToList();
                ...
                return (T)constructor
                    .Invoke(new object[] { id, versionedEvents });
            }
        }
        ...
        public void Save(T eventSourced)
        {
            using (var connection = new SqliteConnection(_connectionString))
            {
                foreach (var pendingEvent in eventSourced.PendingEvents)
                {
                    ...
                    connection.Execute(
                        "INSERT INTO Event (" +
                        "EntityId, EntityType, Version, Payload, Timestamp) VALUES (@EntityId, @EntityType, @Version, @Payload, @Timestamp)",
                        new
                        {
